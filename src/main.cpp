...
#if defined(NDEBUG)
# error "Otacoin cannot be compiled without assertions."
#endif
...
uint256 hashGenesisBlock("0x10bb890c36563cb8cef12d03ae1526969214bb19048a861d5e58bb501f8d8cf5");
static CBigNum bnProofOfWorkLimit(~uint256(0) >> 20); // Otacoin: starting difficulty is 1 / 2^12
...
const string strMessageMagic = "Otacoin Signed Message:\n";
...
{
    // Otacoin: IsDust() detection disabled, allows any valid dust to be relayed.
...
int64 static GetBlockValue(int nHeight, int64 nFees)
{
    ...
    nSubsidy >>= (nHeight / 840000); // Otacoin: 840k blocks in ~4 years

    return nSubsidy + nFees;
}

static const int64 nTargetTimespan = 10 * 60; // Otacoin: 5 minutes
static const int64 nTargetSpacing = 1 * 30; // Otacoin: 30 seconds
...
unsigned int static GetNextWorkRequired(const CBlockIndex* pindexLast, const CBlockHeader *pblock)
{
    ...
    // Otacoin: This fixes an issue where a 51% attack can change difficulty at will.
...
bool CBlock::CheckBlock(CValidationState &state, bool fCheckPOW, bool fCheckMerkleRoot) const
{
    ...
    // Otacoin: Special short-term limits to avoid 10,000 BDB lock limit:
...
bool LoadBlockIndex()
{
    if (fTestNet)
    {
        ...
        hashGenesisBlock = uint256("0x10bb890c36563cb8cef12d03ae1526969214bb19048a861d5e58bb501f8d8cf5");
    }
...
bool InitBlockIndex() {
    ...
    if (!fReindex) {
        ...
        // Genesis block
        const char* pszTimestamp = "i'm pedro but thats not a real name lol";
        CTransaction txNew;
        txNew.vin.resize(1);
        txNew.vout.resize(1);
        ...
        assert(block.hashMerkleRoot == uint256("0xafeeb7fd114ea0a16ec8a1ea9f4491dcafaf480139c5e95fd46e766b75ee1ca6"));
...
unsigned char pchMessageStart[4] = { 0xfb, 0xc0, 0xb6, 0xdb }; // Otacoin: increase each by adding 2 to bitcoin's value.
...
bool CheckWork(CBlock* pblock, CWallet& wallet, CReserveKey& reservekey)
{
    ...
    // Found a solution
    {
        ...
        if (pblock->hashPrevBlock != hashBestChain)
            return error("OtacoinMiner : generated block is stale");
        ...
        if (!ProcessBlock(state, NULL, pblock))
            return error("OtacoinMiner : ProcessBlock, block not accepted");
    }

    return true;
}

void static OtacoinMiner(CWallet *pwallet)
{
    printf("OtacoinMiner started\n");
    SetThreadPriority(THREAD_PRIORITY_LOWEST);
    RenameThread("otacoin-miner");
    ...
    try { loop {
        ...
        printf("Running OtacoinMiner with %"PRIszu" transactions in block (%u bytes)\n", pblock->vtx.size(),
    ...
    {
        printf("OtacoinMiner terminated\n");
        throw;
    }
}
...
void GenerateBitcoins(bool fGenerate, CWallet* pwallet)
{
    ...
    for (int i = 0; i < nThreads; i++)
        minerThreads->create_thread(boost::bind(&OtacoinMiner, pwallet));
}
